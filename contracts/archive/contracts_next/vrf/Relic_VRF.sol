// Relic_VRF.sol - 整合 Chainlink VRF v2.5 的聖物 NFT 合約
    ...
    ConfirmedOwner 
{
    ...
    uint256 public vrfThreshold = 10; // 10個以上使用 VRF
    ...
    function mintFromWallet(uint256 _quantity) external payable nonReentrant whenNotPaused {
        ...
        if (vrfEnabled && _quantity >= vrfThreshold) {
            _requestVRFMint(msg.sender, _quantity, maxRarity, false);
        } else {
    ...
    function mintFromVault(uint256 _quantity) external payable nonReentrant whenNotPaused {
        ...
        if (vrfEnabled && _quantity >= vrfThreshold) {
            _requestVRFMint(msg.sender, _quantity, maxRarity, true);
        } else {
            _executePseudoRandomMint(msg.sender, _quantity, maxRarity);
        }
    }
    ...
    // 請求 VRF 隨機數
    function _requestVRFMint(address recipient, uint256 quantity, uint8 maxRarity, bool fromVault) private {
        uint256 requestId;
        uint256 vrfFee;
    ...
    function _executeVRFMint(uint256 requestId, uint256 baseRandomness) private {
        ...
        for (uint256 i = 0; i < mintData.quantity; i++) {
            ...
            (uint8 rarity, uint8 capacity) = _calculateAttributes(nftRandomness, 0, mintData.maxRarity);
