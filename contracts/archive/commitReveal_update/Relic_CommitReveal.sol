// Relic_CommitReveal.sol - 最小改動的延遲揭示版本
...
contract Relic_CommitReveal is ERC721, Ownable, ReentrancyGuard, Pausable {
    ...
    function revealMint() external nonReentrant whenNotPaused {
        ...
        for (uint256 i = 0; i < commitment.quantity; i++) {
            ...
            userPendingTokens[msg.sender].push(tokenId);
        }
        
        // 立即揭示屬性
        for (uint256 i = 0; i < tokenIds.length; i++) {
            _revealRelic(tokenIds[i], i, commitment.maxRarity, blockHash);
        }
        
        emit BatchMintCompleted(msg.sender, commitment.quantity, commitment.maxRarity, tokenIds);
    }

    // === 新增：單個聖物揭示邏輯 ===
    function _revealRelic(uint256 _tokenId, uint256 _salt, uint8 _maxRarity, bytes32 _blockHash) private {
        ...
        // 從待揭示列表中移除
        _removeFromPending(msg.sender, _tokenId);
        
        emit RelicMinted(_tokenId, msg.sender, rarity, capacity);
        emit RelicRevealed(_tokenId, msg.sender, rarity, capacity);
    }

    // === 新增：從待揭示列表中移除 ===
    function _removeFromPending(address _owner, uint256 _tokenId) private {
        uint256[] storage pending = userPendingTokens[_owner];
        for (uint256 i = 0; i < pending.length; i++) {
            if (pending[i] == _tokenId) {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        if (!relicData[tokenId].isRevealed) {
            return string(abi.encodePacked(unrevealedURI, tokenId.toString()));
        }
