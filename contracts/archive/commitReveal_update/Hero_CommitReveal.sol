// Hero_CommitReveal.sol - 最小改動的延遲揭示版本
...
contract Hero_CommitReveal is ERC721, Ownable, ReentrancyGuard, Pausable {
    ...
    function revealMint() external nonReentrant whenNotPaused {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            _revealHero(tokenIds[i], i, commitment.maxRarity, blockHash);
        }
    ...
    // === 新增：單個英雄揭示邏輯 ===
    function _revealHero(uint256 _tokenId, uint256 _salt, uint8 _maxRarity, bytes32 _blockHash) private {
        ...
        emit HeroMinted(_tokenId, msg.sender, rarity, power);
        emit HeroRevealed(_tokenId, msg.sender, rarity, power);
    }
