// AltarOfAscension_CommitReveal.sol - 最小改動的延遲揭示版本
...
contract AltarOfAscension_CommitReveal is Ownable, ReentrancyGuard, Pausable {
    ...
    // 新增事件
    event UpgradeCommitted(address indexed player, address tokenContract, uint8 baseRarity, uint256 blockNumber);
    ...
    function upgradeNFTs(
        ...
        emit UpgradeCommitted(msg.sender, _tokenContract, baseRarity, block.number);
    }
    ...
    function revealUpgrade() external whenNotPaused nonReentrant {
        ...
        // 再次驗證材料（防止在等待期間NFT被轉移）
        for (uint i = 0; i < commitment.tokenIds.length; i++) {
            if (commitment.tokenContract == address(heroContract)) {
                require(heroContract.ownerOf(commitment.tokenIds[i]) == msg.sender, "Altar: No longer owner");
            } else {
                require(relicContract.ownerOf(commitment.tokenIds[i]) == msg.sender, "Altar: No longer owner");
            }
        }
        
        // 記錄燃燒的代幣ID
        uint256[] memory burnedIds = new uint256[](commitment.tokenIds.length);
        for (uint i = 0; i < commitment.tokenIds.length; i++) {
            burnedIds[i] = commitment.tokenIds[i];
        }
        
        // 執行燃燒
        _burnNFTs(commitment.tokenContract, commitment.tokenIds);
        ...
        (uint256[] memory mintedIds, uint8 outcome, uint8 targetRarity, uint8 vipLevel, uint8 totalVipBonus) = _processUpgradeOutcomeWithReveal(
            msg.sender,
        ...
        // 更新統計
        _updateStats(msg.sender, commitment.tokenIds.length, mintedIds.length, commitment.payment);
        
        // 更新冷卻時間
        lastUpgradeTime[msg.sender][commitment.baseRarity] = block.timestamp;
        ...
        emit UpgradeRevealed(msg.sender, outcome, targetRarity);
    }
