// DungeonMaster_CommitReveal.sol - 最小改動的延遲揭示版本
...
contract DungeonMaster_CommitReveal is Ownable, ReentrancyGuard, Pausable {
    ...
    function revealExpedition() external nonReentrant whenNotPaused {
        ExpeditionCommitment storage commitment = userCommitments[msg.sender];
        ...
        // 再次驗證隊伍所有權（防止在等待期間轉移）
        IParty partyContract = IParty(dungeonCore.partyContractAddress());
        require(partyContract.ownerOf(commitment.partyId) == msg.sender, "DM: No longer party owner");
    ...
    function emergencyRefund() external nonReentrant {
        ...
        require(commitment.blockNumber > 0, "DM: No pending expedition");
        require(!commitment.fulfilled, "DM: Already revealed");
        require(block.number > commitment.blockNumber + REVEAL_BLOCK_DELAY + MAX_REVEAL_WINDOW, "DM: Still in reveal window");
        
        // 清除隊伍冷卻
        PartyStatus memory partyStatus = _getPartyStatus(commitment.partyId);
        partyStatus.cooldownEndsAt = 0;
        _setPartyStatus(commitment.partyId, partyStatus);
