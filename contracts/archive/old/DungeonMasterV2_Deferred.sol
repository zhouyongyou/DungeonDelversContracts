// contracts/DungeonMasterV2_Deferred.sol (延遲發放版本)
...
contract DungeonMasterV2_Deferred is Ownable, ReentrancyGuard, Pausable {
    ...
    function claimRewards(uint256 _partyId) external nonReentrant whenNotPaused {
        ...
        uint256 actualPayout = rewardAmount > contractBalance ? contractBalance : rewardAmount;
        
        if (actualPayout > 0) {
            // 扣除已發放的部分
            status.unclaimedRewards = rewardAmount - actualPayout;
            _setPartyStatus(_partyId, status);
            
            // 轉移獎勵
            soulShardToken.safeTransfer(msg.sender, actualPayout);
            
            emit RewardsBanked(msg.sender, _partyId, actualPayout);
        } else {
            // 餘額不足，無法發放
            revert("DM: Insufficient contract balance, please wait for refill");
        }
    }
    ...
    function getContractBalance() external view returns (uint256) {
        return soulShardToken.balanceOf(address(this));
    }
    
    // ★★★ 新增：查詢總欠款（所有未領取獎勵總和）★★★
    function getTotalOwed() external view returns (uint256) {
        // 這需要遍歷所有隊伍，實際應用中可能需要優化
        // 這裡只是示例，實際可能需要維護一個總數狀態變量
        return 0; // 簡化版本
    }
}