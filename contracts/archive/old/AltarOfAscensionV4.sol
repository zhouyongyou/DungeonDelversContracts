// AltarOfAscensionV2Fixed.sol - 自動VIP加成版本
...
contract AltarOfAscensionV2Fixed is Ownable, ReentrancyGuard, Pausable {
    ...
    // VIP加成
    mapping(address => uint8) public vipBonusRate;
    ...
    function _mintUpgradedNFT(
        ...
        address _tokenContract,
        uint8 _rarity
    ) private returns (uint256) {  // 新增返回值
        if (_tokenContract == address(heroContract)) {
            uint256 power = _generatePowerByRarity(_rarity);
            return heroContract.mintFromAltar(_player, _rarity, power); // 返回 token ID
        } else {
            uint8 capacity = _generateCapacityByRarity(_rarity);
            return relicContract.mintFromAltar(_player, _rarity, capacity); // 返回 token ID
        }
    }
