// AltarOfAscension_UnifiedVRF.sol - 統一使用 VRF 的升級系統
...
contract AltarOfAscension_UnifiedVRF is 
    Ownable, 
    ReentrancyGuard, 
    Pausable, 
    ...
    ConfirmedOwner 
{
    ...
    function _requestVRFUpgrade(
        ...
        // 預先計算 VIP 加成
        (uint8 vipLevel, uint8 totalVipBonus) = _getVIPBonus(_player);
        ...
        override 
    {
        ...
        _processUpgradeWithVRF(requestId, randomness);
        
        emit VRFRequestFulfilled(requestId, randomness);
    }

    // 處理 VRF 升級結果
    function _processUpgradeWithVRF(uint256 _requestId, uint256 _randomness) private {
        PendingUpgrade memory upgrade = pendingUpgrades[_requestId];
        UpgradeRule memory rule = upgradeRules[upgrade.baseRarity];
        
        // 計算升級結果
        uint256 randomValue = _randomness % 100;
        uint256 tempSuccessChance = uint256(rule.successChance) + uint256(upgrade.totalVipBonus);
    ...
    // 獲取升級總費用預估
    function getUpgradeCost(uint8 _baseRarity) external view returns (uint256 totalCost) {
        UpgradeRule memory rule = upgradeRules[_baseRarity];
        totalCost = rule.nativeFee + getVRFFee();
    }
    ...
    function getPlayerVipInfo(address _player) external view returns (
        ...
        (currentVipLevel, totalVipBonus) = _getVIPBonus(_player);
        additionalBonus = additionalVipBonusRate[_player];
        effectiveTotalBonus = totalVipBonus;
    }
    ...
    function previewUpgradeSuccess(address _player, uint8 _baseRarity) external view returns (
        ...
        // 獲取玩家VIP信息
        (playerVipLevel, totalVipBonus) = _getVIPBonus(_player);
        ...
        totalCost = getUpgradeCost(_baseRarity);
    }
