// Hero_UnifiedVRF.sol - 統一使用 VRF，統一稀有度機率
    ...
    ConfirmedOwner 
{
    ...
    function mintFromWallet(uint256 _quantity) external payable nonReentrant whenNotPaused {
        require(_quantity > 0 && _quantity <= 50, "Hero: Invalid quantity (1-50)");
    ...
    function _executeVRFMint(uint256 requestId, uint256 baseRandomness) private {
        ...
        for (uint256 i = 0; i < mintData.quantity; i++) {
            ...
            // 使用統一稀有度機率計算屬性
            (uint8 rarity, uint256 power) = _calculateAttributesUnified(nftRandomness);
    ...
    function _calculateAttributesUnified(uint256 _randomNumber) private view returns (uint8 rarity, uint256 power) {
    ...
    function emergencyMint(
        ...
        for (uint256 i = 0; i < quantity; i++) {
            ...
            uint256 power = _generateHeroPowerByRarity(fixedRarity, emergencyRandom);
            
            tokenIds[i] = _mintHero(recipient, fixedRarity, power);
        }
