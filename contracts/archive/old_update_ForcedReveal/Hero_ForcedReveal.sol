// Hero_ForcedReveal.sol - 過期強制揭示版本（移除退款機制）
...
contract Hero is ERC721, Ownable, ReentrancyGuard, Pausable {
    ...
    function _revealHeroForced(uint256 _tokenId, address _owner) private {
        // 懲罰性結果：固定稀有度 1，隨機戰力範圍最低
        uint8 rarity = 1;
        uint256 power = 15 + (uint256(keccak256(abi.encodePacked(block.timestamp, _tokenId, _owner))) % 36); // 15-50
