...
import "../../../current/interfaces/interfaces.sol";
...
/**
 * @title Oracle_Final
 * @notice DungeonDelvers 最終版 Oracle 合約
 * @dev 整合了所有必要的功能，提供完整的價格查詢介面
 */
contract Oracle_Final is Ownable {
    using MulDiv for uint256;

    // ========== 狀態變數 ==========
    IUniswapV3Pool public immutable pool;
    address public immutable poolAddress;      // 公開的 pool 地址
    address public immutable soulShardToken;   // SoulShard token 地址
    address public immutable usdToken;         // USD token 地址
    address public immutable token0;           // Pool 的 token0
    address public immutable token1;           // Pool 的 token1
    bool private immutable isSoulShardToken0;  // SoulShard 是否為 token0
    
    uint32 public twapPeriod;                  // TWAP 週期（秒）
     ...
     * @param usdAmount USD 金額（18 位小數）
     * @return soulAmount 需要的 SoulShard 數量（18 位小數）
     */
    function getRequiredSoulShardAmount(uint256 usdAmount) public view returns (uint256 soulAmount) {
        uint256 soulShardPrice = getSoulShardPriceInUSD();
        require(soulShardPrice > 0, "Oracle: ZERO_PRICE");
        // soulAmount = usdAmount / price (USD/SOUL)
    ...
    /**
     * @notice 設置 TWAP 週期
     * @param _newTwapPeriod 新的 TWAP 週期（秒）
     */
