...
/**
 * @title Relic_Secured
 * @notice 聖物 NFT 合約 - 安全加固版
 * @dev 安全改進：
 * 1. 添加批量鑄造數量限制
 * 2. 每個區塊的鑄造限制防止 DoS
 * 3. 所有涉及資金的函數確保有 nonReentrant
 * 4. 加強輸入驗證
 */
contract Relic_Secured is ERC721, Ownable, ReentrancyGuard, Pausable {
    ...
    function mintFromWallet(uint256 _quantity) external payable nonReentrant whenNotPaused {
        // ★ 安全加固：批量數量限制
        require(_quantity > 0 && _quantity <= MAX_BATCH_SIZE, "Relic: Invalid quantity");
        require(msg.value >= platformFee * _quantity, "Relic: Platform fee not met");
        
        // ★ 安全加固：每個區塊的鑄造限制
        uint256 currentBlock = block.number;
        require(mintedPerBlock[currentBlock] + _quantity <= MAX_BATCH_SIZE_PER_BLOCK, "Relic: Block mint limit exceeded");
        mintedPerBlock[currentBlock] += _quantity;
        
        require(address(dungeonCore) != address(0), "Relic: DungeonCore not set");
        require(address(soulShardToken) != address(0), "Relic: SoulShard token not set");
    ...
    function _generateAndMintOnChain(address _to, uint256 _salt, uint8 _maxRarity) private returns (uint256) {
        ...
        return _mintRelic(_to, rarity, capacity);
    }
    ...
    function setBatchTier(uint256 _tierId, uint256 _minQuantity, uint8 _maxRarity, string memory _tierName) external onlyOwner nonReentrant {
        ...
        require(_minQuantity <= MAX_BATCH_SIZE, "Relic: Min quantity exceeds max batch size");
