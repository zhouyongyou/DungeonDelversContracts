...
contract Oracle_V22_Adaptive_Secured is Ownable, ReentrancyGuard {
    ...
    uint256 public constant MAX_PRICE_CHANGE_BPS = 2000;  // 20% in basis points
    ...
    event PriceChangeRejected(uint256 oldPrice, uint256 newPrice, uint256 changeBps);
    event EmergencyPriceOverride(uint256 newPrice);
    ...
    function getPriceAdaptive() public view returns (uint256 price, uint32 usedPeriod) {
        for (uint i = 0; i < adaptivePeriods.length; i++) {
            // ★ 安全加固：跳過小於最小安全週期的選項
            if (adaptivePeriods[i] < MIN_SAFE_PERIOD) {
                continue;
            }
            ...
            if (success) {
                // ★ 安全加固：檢查價格變動幅度
                if (_isPriceChangeAcceptable(adaptivePrice)) {
                    return (adaptivePrice, adaptivePeriods[i]);
                }
            }
        }
        
        // 如果所有週期都失敗或價格變動過大，返回上次記錄的價格（如果有）
        require(lastRecordedPrice > 0, "Oracle: No valid price available");
        return (lastRecordedPrice, 0); // 0 表示使用了緩存價格
    }
    
    /**
     * @notice 檢查價格變動是否在可接受範圍內
     * @param newPrice 新價格
     * @return 是否可接受
     */
    function _isPriceChangeAcceptable(uint256 newPrice) internal view returns (bool) {
        if (lastRecordedPrice == 0) {
            return true; // 第一次設置價格
        }
        
        // 計算價格變動百分比（基點）
        uint256 priceDiff = newPrice > lastRecordedPrice 
            ? newPrice - lastRecordedPrice 
            : lastRecordedPrice - newPrice;
            
        uint256 changeBps = (priceDiff * 10000) / lastRecordedPrice;
        
        return changeBps <= MAX_PRICE_CHANGE_BPS;
    }
    ...
    function testAllPeriods() external view returns (bool[] memory available, uint256[] memory prices) {
        ...
        for (uint i = 0; i < adaptivePeriods.length; i++) {
            ...
            available[i] = success && (adaptivePeriods[i] >= MIN_SAFE_PERIOD);
            if (available[i]) {
                prices[i] = price;
            }
        }
     ...
     * @notice 獲取價格健康狀態
     */
    function getPriceHealth() external view returns (
        uint256 currentPrice,
        uint256 lastPrice,
        uint256 timeSinceLastUpdate,
        bool isHealthy
    ) {
        (currentPrice, ) = getPriceAdaptive();
        lastPrice = lastRecordedPrice;
        timeSinceLastUpdate = block.timestamp - lastPriceUpdateTime;
        isHealthy = _isPriceChangeAcceptable(currentPrice);
    }
     ...
     */
    function setTwapPeriod(uint32 _newTwapPeriod) external onlyOwner nonReentrant {
        require(_newTwapPeriod >= MIN_SAFE_PERIOD, "Oracle: Period below minimum");
     ...
     */
    function setAdaptivePeriods(uint32[] calldata _periods) external onlyOwner nonReentrant {
        require(_periods.length > 0 && _periods.length <= 10, "Oracle: Invalid periods length");
        
        // 驗證降序排列且都大於等於最小安全週期
        for (uint i = 0; i < _periods.length; i++) {
            require(_periods[i] >= MIN_SAFE_PERIOD, "Oracle: Period below minimum");
            if (i > 0) {
     ...
     * @dev 僅在極端情況下使用
     */
    function emergencyUpdatePrice() external onlyOwner nonReentrant {
        (uint256 price, ) = getPriceAdaptive();
        lastRecordedPrice = price;
        lastPriceUpdateTime = block.timestamp;
        emit EmergencyPriceOverride(price);
    }
     ...
     * @dev 極端緊急情況下使用
     */
    function emergencySetPrice(uint256 _price) external onlyOwner nonReentrant {
        require(_price > 0, "Oracle: Invalid price");
        lastRecordedPrice = _price;
        lastPriceUpdateTime = block.timestamp;
        emit EmergencyPriceOverride(_price);
    }
