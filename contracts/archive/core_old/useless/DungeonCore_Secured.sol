 ...
 */
contract DungeonCore_Secured is Ownable, ReentrancyGuard {
    ...
    /**
     * @notice 從 PlayerVault 扣款
     * @dev 安全加固：添加 nonReentrant 防止重入攻擊
     */
    ...
    // --- Owner 管理函式 (添加 nonReentrant 保護) ---
    
    function setOracle(address _newAddress) external onlyOwner nonReentrant {
        require(_newAddress != address(0), "Cannot set zero address");
        oracleAddress = _newAddress;
        emit OracleSet(_newAddress);
    }

    function setPlayerVault(address _newAddress) external onlyOwner nonReentrant {
        require(_newAddress != address(0), "Cannot set zero address");
        playerVaultAddress = _newAddress;
        emit PlayerVaultSet(_newAddress);
    }

    function setDungeonMaster(address _newAddress) external onlyOwner nonReentrant {
        require(_newAddress != address(0), "Cannot set zero address");
        dungeonMasterAddress = _newAddress;
        emit DungeonMasterSet(_newAddress);
    }

    function setAltarOfAscension(address _newAddress) external onlyOwner nonReentrant {
        require(_newAddress != address(0), "Cannot set zero address");
        altarOfAscensionAddress = _newAddress;
        emit AltarOfAscensionSet(_newAddress);
    }

    function setHeroContract(address _newAddress) external onlyOwner nonReentrant {
        require(_newAddress != address(0), "Cannot set zero address");
        heroContractAddress = _newAddress;
        emit HeroContractSet(_newAddress);
    }

    function setRelicContract(address _newAddress) external onlyOwner nonReentrant {
        require(_newAddress != address(0), "Cannot set zero address");
        relicContractAddress = _newAddress;
        emit RelicContractSet(_newAddress);
    }

    function setPartyContract(address _newAddress) external onlyOwner nonReentrant {
        require(_newAddress != address(0), "Cannot set zero address");
        partyContractAddress = _newAddress;
        emit PartyContractSet(_newAddress);
    }

     function setPlayerProfile(address _newAddress) external onlyOwner nonReentrant {
        require(_newAddress != address(0), "Cannot set zero address");
        playerProfileAddress = _newAddress;
        emit PlayerProfileSet(_newAddress);
    }

    function setVipStaking(address _newAddress) external onlyOwner nonReentrant {
        require(_newAddress != address(0), "Cannot set zero address");
