// Relic_UnifiedRates.sol - 統一機率版本（所有數量都使用50個的機率）
...
contract Relic is ERC721, Ownable, ReentrancyGuard, Pausable {
    ...
    function _revealRelic(uint256 _tokenId, uint256 _salt, uint8 _maxRarity, bytes32 _blockHash, address _owner) private {
        ...
        uint8 capacity = rarity; // 聖物的容量等於稀有度
    ...
    function _calculateAttributes(uint256 _randomNumber, uint8 _fixedRarity, uint8 _maxRarity) private view returns (uint8 rarity, uint8 capacity) {
        if (_fixedRarity > 0) {
            rarity = _fixedRarity;
        } else {
