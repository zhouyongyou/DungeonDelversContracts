...
contract Relic_DelayedReveal is ERC721, ERC721Enumerable, ERC721Burnable, ERC721Pausable, Ownable, ReentrancyGuard {
    ...
    string public unrevealedURI = "ipfs://QmUnrevealedRelic/";
    
    uint256 public mintPriceUSD = 10**18; // $1 USD
    ...
    // --- 事件 ---
    event RelicCommitted(address indexed player, uint256 quantity, uint256 blockNumber, bytes32 commitment);
    event RelicMinted(uint256 indexed tokenId, address indexed owner, uint256 blockNumber);
    event RelicRevealed(uint256 indexed tokenId, address indexed owner, uint8 rarity, uint256 effect);
    ...
    event RelicBurned(uint256 indexed tokenId, address indexed owner, uint8 rarity, uint256 effect);
    ...
    constructor(address _initialOwner) Ownable(_initialOwner) ERC721("Dungeon Delvers Relic", "RELIC") {
        contractURI = "https://api.dungeondelvers.com/metadata/reliccontract.json";
    ...
    function commitMintFromWallet(uint256 _quantity, bytes32 _commitment) external payable nonReentrant whenNotPaused {
        ...
        require(userCommitments[msg.sender].blockNumber == 0 || userCommitments[msg.sender].revealed, "Relic: Previous mint pending");
        ...
        emit RelicCommitted(msg.sender, _quantity, block.number, _commitment);
    }
    ...
    function revealMint(uint256 _nonce) external nonReentrant whenNotPaused {
        ...
        require(!commitment.revealed, "Relic: Already revealed");
        ...
        require(calculatedCommitment == commitment.commitment, "Relic: Invalid reveal");
        ...
        for (uint256 i = 0; i < commitment.quantity; i++) {
            ...
            emit RelicMinted(tokenId, msg.sender, block.number);
        }
    ...
    function _revealBatch(uint256[] memory tokenIds, uint256 baseRandom, uint8 maxRarity) private {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            (uint8 rarity, uint256 effect) = _calculateAttributes(tokenRandom, 0, maxRarity);
            ...
            emit RelicRevealed(tokenId, ownerOf(tokenId), rarity, effect);
        }
    ...
    function emergencyRefund() external nonReentrant {
        ...
        require(commitment.blockNumber > 0, "Relic: No pending mint");
        require(!commitment.revealed, "Relic: Already revealed");
        require(block.number > commitment.blockNumber + REVEAL_BLOCK_DELAY + MAX_REVEAL_WINDOW, "Relic: Still in reveal window");
    ...
    // 祭壇鑄造（立即揭示）
    function mintFromAltar(address _to, uint8 _rarity, uint256 _effect) external onlyAltar returns (uint256) {
        uint256 tokenId = _nextTokenId++;
        relicData[tokenId] = RelicData({
            rarity: _rarity,
            effect: _effect,
        ...
        emit RelicMinted(tokenId, _to, block.number);
        emit RelicRevealed(tokenId, _to, _rarity, _effect);
        return tokenId;
    }
    ...
    function burnFromAltar(uint256 _tokenId) external onlyAltar {
        ...
        emit RelicBurned(_tokenId, owner, data.rarity, data.effect);
        _burn(_tokenId);
    }

    function _calculateAttributes(uint256 _randomNumber, uint8 _fixedRarity, uint8 _maxRarity) private pure returns (uint8 rarity, uint256 effect) {
        if (_fixedRarity > 0) {
            rarity = _fixedRarity;
        } else {
            rarity = _calculateRarityWithCap(_randomNumber, _maxRarity);
        }
        effect = _generateRelicEffectByRarity(rarity, _randomNumber >> 8);
    }
    ...
    function _generateRelicEffectByRarity(uint8 _rarity, uint256 _randomNumber) private pure returns (uint256 effect) {
        if (_rarity == 1) { effect = 5 + (_randomNumber % (20 - 5 + 1)); }
        else if (_rarity == 2) { effect = 20 + (_randomNumber % (50 - 20 + 1)); }
        else if (_rarity == 3) { effect = 50 + (_randomNumber % (100 - 50 + 1)); }
        else if (_rarity == 4) { effect = 100 + (_randomNumber % (150 - 100 + 1)); }
        else if (_rarity == 5) { effect = 150 + (_randomNumber % (200 - 150 + 1)); }
    ...
    function getRelicData(uint256 _tokenId) external view returns (uint8 rarity, uint256 effect, bool isRevealed) {
        RelicData memory data = relicData[_tokenId];
        return (data.rarity, data.effect, data.isRevealed);
    }

    function getRequiredSoulShardAmount(uint256 _quantity) public view returns (uint256) {
        require(address(dungeonCore) != address(0), "Relic: Dungeon Core not set");
        address oracleAddress = dungeonCore.oracleAddress();
        require(oracleAddress != address(0), "Relic: Oracle not set in Dungeon Core");
