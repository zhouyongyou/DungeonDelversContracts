...
contract Hero is ERC721, Ownable, ReentrancyGuard, Pausable {
    ...
    function _executeReveal(address user, bool isForced) private {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            if (isForced) {
                // 強制揭示：固定給最低稀有度作為懲罰
                _revealHeroForced(tokenIds[i], user);
            } else {
                // 正常揭示：使用統一機率邏輯
    ...
    // === 新增：強制揭示邏輯（懲罰性結果）===
    function _revealHeroForced(uint256 _tokenId, address _owner) private {
        // 懲罰性結果：固定稀有度 1，最低戰力
        uint8 rarity = 1;
        uint256 power = 15 + (uint256(keccak256(abi.encodePacked(_tokenId))) % 36); // 15-50
    ...
    function _calculateAttributes(uint256 _randomNumber, uint8 _fixedRarity, uint8 _maxRarity) private view returns (uint8 rarity, uint256 power) {
        if (_fixedRarity > 0) {
            rarity = _fixedRarity;
        } else {
            // 這個函數在新版本不應該被調用，但為了兼容性保留
            rarity = _calculateRarityUnified(_randomNumber, msg.sender, 1);
        }
    ...
    // 刪除原本的 _calculateRarityWithCap 函數，改用統一機率
