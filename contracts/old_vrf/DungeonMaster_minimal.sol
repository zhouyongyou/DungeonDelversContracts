// DungeonMaster_minimal.sol - 最小化改動版本（移除複雜退款機制）
...
contract DungeonMaster_Minimal is Ownable, ReentrancyGuard, Pausable, IVRFCallback {
    ...
    event RequestReset(address indexed user);
    // === VRF 事件 ===
    ...
    // 🎯 新增：用戶自主重置功能（替代複雜的清理機制）
    function resetFailedRequest() external {
        require(userRequests[msg.sender].player != address(0), "DM: No request found");
        require(!userRequests[msg.sender].fulfilled, "DM: Request already fulfilled");
        
        // 允許用戶自己重置失敗的請求
        delete userRequests[msg.sender];
        
        emit RequestReset(msg.sender);
    }
