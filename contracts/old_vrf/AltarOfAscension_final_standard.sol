// AltarOfAscension_final_standard.sol - 標準回調 + 清理邏輯
...
contract AltarOfAscension_Final_Standard is Ownable, ReentrancyGuard, Pausable, IVRFCallback {
    ...
    // === 🎯 標準 VRF 回調實現（學習 NFT 合約 + DungeonMaster 安全性）===
    ...
    function _processUpgradeWithVRF(
        ...
        for (uint256 i = 0; i < request.burnedTokenIds.length; i++) {
            if (IERC721(request.tokenContract).ownerOf(request.burnedTokenIds[i]) != user) {
                // 如果 NFT 已轉移，升級失敗但不回滾狀態
