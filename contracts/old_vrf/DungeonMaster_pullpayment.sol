// DungeonMaster_pullpayment.sol - Pull Payment 安全版本
...
contract DungeonMaster_PullPayment is Ownable, ReentrancyGuard, Pausable, IVRFCallback {
    ...
    mapping(uint256 => uint256) public requestTimestamp; // requestId => timestamp for timeout
        ...
        external payable nonReentrant whenNotPaused
    {
        ...
        if (vrfManager != address(0)) {
            ...
            // Record timestamp for timeout handling
            // Note: Need to get requestId from VRF call above
            // This is a simplified approach - in production would track properly
    ...
    function _processExpeditionResultWithVRF(address _requester, uint256 _partyId, uint256 _dungeonId, uint256 _randomWord) private {
        ...
        // Enhanced random with multiple entropy sources
        uint256 enhancedRandom = uint256(keccak256(abi.encodePacked(
            _randomWord,
            _requester,
            _partyId,
            block.timestamp,
        ...
        // 使用增強隨機數生成結果
        uint256 randomValue = enhancedRandom % 100;
    ...
    // === PULL PAYMENT 實施：超時清理函數 ===
    function cleanupExpiredRequest(address user) external {
        ExpeditionRequest storage request = userRequests[user];
        require(request.player != address(0), "No request");
        require(!request.fulfilled, "Already fulfilled");
        // Note: Would need proper timestamp tracking per request
        // This is simplified - in production would check actual request timestamp
        
        // Reset request
        delete userRequests[user];
        
        // Record refund if payment was made (Pull Payment)
        if (request.payment > 0) {
            // PULL PAYMENT: 記帳而非直接轉帳
    ...
    function onVRFFulfilled(uint256 requestId, uint256[] memory randomWords) external override {
        // Don't revert - use return instead
        if (msg.sender != vrfManager) return;
        if (randomWords.length == 0) return;
        
        // Check timeout
        if (requestTimestamp[requestId] != 0 && 
            block.timestamp > requestTimestamp[requestId] + REQUEST_TIMEOUT) {
