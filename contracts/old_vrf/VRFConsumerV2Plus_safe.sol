 ...
 * 4. 改進錯誤處理和事件記錄
 */
contract VRFConsumerV2Plus_Safe is VRFConsumerBaseV2Plus, ReentrancyGuard {
    ...
    uint16 public requestConfirmations = 8; // Optimized for $200-400 NFT value (24 seconds)
    ...
    uint256 public constant REQUEST_TIMEOUT = 1 hours; // Request timeout
    ...
    function fulfillRandomWords(
        ...
        // Don't revert - use return instead to prevent VRF failure
        if (!s_requests[_requestId].exists) return;
        if (s_requests[_requestId].fulfilled) return;
        
        // Check timeout
        if (block.timestamp > s_requests[_requestId].timestamp + REQUEST_TIMEOUT) return;
