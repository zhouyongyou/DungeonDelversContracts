// AltarOfAscension_final.sol - 最終統一版本（混合權限模式）
...
contract AltarOfAscension_Final is Ownable, ReentrancyGuard, Pausable, IVRFCallback {
    ...
    function upgradeNFTs(
        ...
        if (vrfManager != address(0)) {
            ...
            requestTimestamp[requestId] = block.timestamp; // Record timestamp
    ...
    function onVRFFulfilled(
        ...
        // Check timeout
        if (block.timestamp > requestTimestamp[requestId] + REQUEST_TIMEOUT) {
    ...
    // === 混合權限超時清理函數（方案 A 實施）===
    // 注意：AltarOfAscension 不需要退款功能，只需解鎖 tokens
    function cleanupExpiredRequest(address user) external {
        uint256 requestId = activeUpgradeRequest[user];
        require(requestId != 0, "No active request");
        require(block.timestamp > requestTimestamp[requestId] + REQUEST_TIMEOUT, "Not expired");
        
        // 混合權限檢查：
        // 1. 用戶自己可以清理（最優先）
        // 2. Owner 可以清理（管理員權限）
        // 3. 2倍超時後任何人可以清理（防止永久鎖定）
        require(
            msg.sender == user ||  // 用戶自己
            msg.sender == owner() || // Owner
            block.timestamp > requestTimestamp[requestId] + REQUEST_TIMEOUT * 2, // 1小時後公開
            "Not authorized to cleanup"
        );
        
        // Unlock tokens if any were locked
        // Note: Would need to track which tokens were locked per request for complete cleanup
