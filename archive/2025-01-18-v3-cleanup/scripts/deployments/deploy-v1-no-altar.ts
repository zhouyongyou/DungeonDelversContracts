// scripts/deploy-v1-no-altar.ts (V1版本 - 無祭壇功能)
...
const CONFIG_PATHS = {
    // 統一配置文件
    unifiedConfig: path.join(__dirname, "../contract-config-v1.json"),
...
async function main() {
    log("🚀 正在部署 Dungeon Delvers V1 核心遊戲合約 (無祭壇功能)...");
    ...
    // ⚠️ V1版本：將祭壇地址設為零地址
    await (await dc.setAltarOfAscension("0x0000000000000000000000000000000000000000")).wait();
    ...
    // 移除疲勞度系統 - 註釋掉 setRestCostPowerDivisor 調用
    ...
    // --- 最終報告 ---
    log("🎉🎉🎉 恭喜！Dungeon Delvers V1 (無祭壇功能) 部署完成！ 🎉🎉🎉");
    
    console.log("\n=== 🚀 V1 版本部署報告 ===");
    console.log("✅ 核心功能已部署（鑄造、組隊、探險）");
    console.log("⚠️  祭壇功能已暫時禁用");
    ...
    console.log("🎯 下一步：");
    console.log("1. 在前端代碼中註釋掉祭壇相關頁面和功能");
    console.log("2. 更新子圖，移除祭壇相關的事件監聽");
    console.log("3. 重新部署前端到 Vercel");
    console.log("4. 重新部署子圖到 The Graph");
    console.log("5. 重新部署後端到 Render");
    console.log("6. 測試核心功能（鑄造、組隊、探險）");
    console.log("\n⚠️  重要提醒：");
    console.log("- 確保前端不會嘗試調用祭壇合約");
    console.log("- 確保子圖不會監聽祭壇事件");
    console.log("- 祭壇地址已設為零地址：0x0000000000000000000000000000000000000000");
}
