    ...
    async syncABI() {
        ...
        for (const contract of abiContracts) {
            try {
                ...
                // 尋找 ABI 文件，使用正確的 artifact 名稱
                const possiblePaths = [
                    path.join(this.abiPaths.source, `current/nft/${name}.sol/${artifactName}.json`),
                    path.join(this.abiPaths.source, `current/core/${name}.sol/${artifactName}.json`),
                    path.join(this.abiPaths.source, `current/core/AltarOfAscension.sol/${artifactName}.json`), // 特殊處理 AltarOfAscension
                    path.join(this.abiPaths.source, `current/nft/Party.sol/${artifactName}.json`), // 特殊處理 Party
                    path.join(this.abiPaths.source, `current/game/${name}.sol/${artifactName}.json`)
                ];
                ...
                for (const artifactPath of possiblePaths) {
                    if (fs.existsSync(artifactPath)) {
                        ...
                        // 保存到輸出目錄（使用標準化名稱）
                        const outputPath = path.join(this.abiPaths.output, `${name}.json`);
                        ...
                        log(`    從: ${path.basename(artifactPath)}`, 'dim');
                        successCount++;
                        abiFound = true;
                        break;
                    }
                ...
                if (!abiFound) {
                    ...
                    log(`  ⚠️  ${name}: 未找到 artifact`, 'yellow');
                    
                    // 調試信息：顯示嘗試的路徑
                    log(`    嘗試的路徑：`, 'dim');
                    possiblePaths.forEach(p => {
                        log(`      - ${p.replace(this.abiPaths.source, '...')}`, 'dim');
                    });
                }
