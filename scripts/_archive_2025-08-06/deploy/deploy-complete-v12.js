// scripts/deploy-v12-dungeons-update.js
// 基於 V11，更新地城數據為正確的配置
...
function saveAllAddresses(addresses, network, blockNumber) {
  ...
  let envContent = `# DungeonDelvers V12 Deployment (Updated Dungeons)\n`;
...
async function main() {
  log('\n🚀 Starting DungeonDelvers V12 Deployment (Updated Dungeons)', 'bright');
  log('========================================================\n', 'bright');
  ...
  try {
    ...
    const Oracle = await ethers.getContractFactory("contracts/defi/Oracle.sol:Oracle");
    ...
    await playerVault.setSoulShardToken(addresses.SOULSHARD_ADDRESS);
    log('  ✅ PlayerVault -> SoulShardToken');
    ...
    // 初始化地城數據（V12 更新版本）
    log('\n🏰 Initializing dungeons with updated data...', 'cyan');
    ...
    for (const dungeon of dungeons) {
      ...
      log(`  ✅ Dungeon ${dungeon.id} "${dungeon.name}" initialized (Power: ${dungeon.requiredPower}, Reward: $${ethers.formatEther(dungeon.rewardAmountUSD)}, Success: ${dungeon.baseSuccessRate}%)`);
    }
    ...
    log('\n🏰 Initialized Dungeons:', 'green');
    dungeons.forEach(d => {
      log(`  ${d.id}. ${d.name} - Power: ${d.requiredPower}, Reward: $${ethers.formatEther(d.rewardAmountUSD)}`, 'cyan');
    });
