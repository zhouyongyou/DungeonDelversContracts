...
const CONFIG = {
  ...
  SLIPPAGE_PERCENT: 2.0,             // 滑點保護 (%) - 增加到 2%
...
const ADDRESSES = {
  ...
  PANCAKE_V3_ROUTER: '0x1b81D678ffb9C0263b24A97847620C99d213eB14',  // 正確的 V3 Router
...
async function initialize() {
  ...
  log(`\n📌 池子配置:`, 'cyan');
  log(`   Token0: ${token0} (${token0 === ADDRESSES.USD ? 'USD' : 'SOUL'})`, 'white');
  log(`   Token1: ${token1} (${token1 === ADDRESSES.SOUL ? 'SOUL' : 'USD'})`, 'white');
  log(`   Fee: ${await pool.fee() / 10000}%`, 'white');
}
...
async function checkPoolLiquidity() {
  try {
    ...
    log(`\n📊 池子流動性狀態:`, 'cyan');
    log(`   總流動性: ${liquidity.toString()}`, 'white');
    log(`   當前 tick: ${slot0.tick}`, 'white');
    
    if (liquidity === 0n) {
      log('⚠️  警告：池子流動性為零！', 'red');
      return false;
    }
    
    return true;
  } catch (error) {
    log(`❌ 檢查流動性失敗: ${error.message}`, 'red');
    return false;
  }
}
...
async function getQuote(tokenIn, tokenOut, amountIn, fee) {
  try {
    ...
    if (tokenIn === ADDRESSES.USD) {
      // USD -> SOUL: 假設 1 USD = 17000 SOUL
      return amountIn * 17000n / ethers.parseUnits('1', 18);
    } else {
      // SOUL -> USD: 假設 17000 SOUL = 1 USD
      return amountIn * ethers.parseUnits('1', 18) / 17000n;
    }
  }
}
...
async function executeTrade(isBuy) {
  ...
  try {
    ...
    if (isBuy) {
      ...
      log(`\n💱 買入交易詳情:`, 'cyan');
      log(`   輸入: ${CONFIG.TRADE_AMOUNT_USD} USD`, 'white');
      log(`   預期獲得: ${ethers.formatUnits(expectedOutput, 18)} SOUL`, 'white');
      log(`   最小接受: ${ethers.formatUnits(minOutput, 18)} SOUL (${CONFIG.SLIPPAGE_PERCENT}% 滑點)`, 'yellow');
      
      // 執行交易
      log(`\n執行買入: ${CONFIG.TRADE_AMOUNT_USD} USD → SOUL`, 'green');
      ...
      log(`\n💱 賣出交易詳情:`, 'cyan');
      log(`   輸入: ${CONFIG.TRADE_AMOUNT_SOUL} SOUL`, 'white');
      log(`   預期獲得: ${ethers.formatUnits(expectedOutput, 18)} USD`, 'white');
      log(`   最小接受: ${ethers.formatUnits(minOutput, 18)} USD (${CONFIG.SLIPPAGE_PERCENT}% 滑點)`, 'yellow');
      
      // 執行交易
      log(`\n執行賣出: ${CONFIG.TRADE_AMOUNT_SOUL} SOUL → USD`, 'red');
