// scripts/deploy-main.ts (最終版 - 支援 BaseURI)
...
async function main() {
    ...
    if (!SOUL_SHARD_TOKEN_ADDRESS || !USD_TOKEN_ADDRESS || !POOL_ADDRESS || !METADATA_SERVER_BASE_URL) {
        throw new Error("❌ 錯誤：請務必在 .env 檔案中提供所有必要的地址，包括 METADATA_SERVER_BASE_URL。");
    }
    
    const finalOwner = FINAL_OWNER_ADDRESS || deployer.address;
    
    logInfo(`最終擁有者地址: ${finalOwner}`);
    logInfo(`元數據伺服器 Base URL: ${METADATA_SERVER_BASE_URL}`);
    // ★★★【修正】★★★ 重新加入重要的日誌，用於部署前確認
    ...
    for (const name of nftContracts) {
        ...
        logInfo(`正在為 ${name} 設定 BaseURI 為: ${uri}`);
        await (await contractInstance.setBaseURI(uri)).wait();
    }
