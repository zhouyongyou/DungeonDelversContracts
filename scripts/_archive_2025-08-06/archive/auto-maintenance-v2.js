...
const CONFIG = {
  ...
  SLIPPAGE_PERCENT: 1.5,       // 滑點容忍度 1.5%
  AUTO_MODE: true,             // 自動模式
...
async function calculateMinOutput(provider, amountIn, tokenIn, tokenOut) {
  try {
    ...
    if (tokenIn === ADDRESSES.USD && tokenOut === ADDRESSES.SOUL) {
      // USD -> SOUL
      const expectedAmount = await oracle.getRequiredSoulShardAmount(amountIn);
      // 將滑點百分比轉換為基點 (basis points)
      ...
      // SOUL -> USD (簡化計算)
      // 假設 1 USD = 16000 SOUL，反向計算
      const expectedUSD = amountIn / 16000n;
      // 將滑點百分比轉換為基點 (basis points)
      const slippageFactor = BigInt(Math.floor((100 - CONFIG.SLIPPAGE_PERCENT) * 100));
      const minUSD = expectedUSD * slippageFactor / 10000n;
      
      return minUSD;
    }
  } catch (error) {
    log(`無法計算預期輸出，使用 0: ${error.message}`, 'yellow');
    return 0n;
  }
}
  ...
  try {
    ...
    if (direction === 'BUY') {
      ...
      const params = {
        ...
        amountIn: amount,
        amountOutMinimum: minOutput,  // 添加滑點保護
        sqrtPriceLimitX96: 0
      };
      ...
      // SOUL -> USD
      const soulAmount = ethers.parseUnits('8000', 18);
...
async function showStatus(signer) {
  ...
  console.log(`   滑點設置: ${CONFIG.SLIPPAGE_PERCENT}%`);
  console.log(`\n💰 當前餘額:`);
