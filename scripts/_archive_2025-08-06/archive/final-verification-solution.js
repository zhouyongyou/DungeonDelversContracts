...
const contracts = {
  Oracle: {
    ...
    flattenedFile: "Oracle_VerificationFix_flattened.sol",
    contractName: "Oracle"
  },
  DungeonCore: {
    ...
    flattenedFile: "DungeonCore_VerificationFix_flattened.sol",
...
// 使用扁平化源碼驗證
async function verifyWithFlattenedSource(name, contractInfo) {
  try {
    log(`\n🔄 使用扁平化源碼驗證 ${name}...`, 'yellow');
    
    const sourceCode = readFlattenedSource(contractInfo.flattenedFile);
    log(`📁 讀取扁平化源碼: ${contractInfo.flattenedFile}`, 'cyan');
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      if (response.data.status === '1') {
        ...
        // 提供手動驗證信息
        log(`📋 手動驗證信息:`, 'magenta');
        log(`   網址: https://bscscan.com/verifyContract`, 'cyan');
        log(`   地址: ${contracts[name].address}`, 'cyan');
        log(`   編譯器: v0.8.20+commit.a1b79de6`, 'cyan');
        log(`   優化: Yes, 200 runs`, 'cyan');
        log(`   構造參數: ${contracts[name].constructorParams}`, 'cyan');
        log(`   使用扁平化文件: ${contracts[name].flattenedFile}`, 'cyan');
...
async function main() {
  ...
  for (const [name, info] of Object.entries(contracts)) {
    ...
    if (!fs.existsSync(filePath)) {
      log(`❌ 扁平化文件不存在: ${info.flattenedFile}`, 'red');
      log(`請先運行: npx hardhat flatten contracts/.../${name}.sol > ${info.flattenedFile}`, 'yellow');
      process.exit(1);
    }
    log(`✅ 找到扁平化文件: ${info.flattenedFile}`, 'green');
  }
  
  const results = [];
  
  for (const [name, contractInfo] of Object.entries(contracts)) {
    const verified = await verifyWithFlattenedSource(name, contractInfo);
  ...
  if (failed === 0) {
    ...
    log('\n💡 對於失敗的合約，建議手動在 BSCScan 上驗證', 'yellow');
    log('   使用上面提供的扁平化文件和參數信息', 'yellow');
  }
