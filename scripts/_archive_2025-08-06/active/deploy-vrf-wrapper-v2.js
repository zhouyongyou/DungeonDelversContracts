// deploy-vrf-wrapper-v2.js - 部署 VRFManagerWrapperV2
...
async function main() {
    console.log("🚀 部署 VRFManagerWrapperV2 (Direct Funding with VRF Wrapper)...\n");
    ...
    try {
        // 讀取編譯後的合約
        const artifactPath = path.join(__dirname, '../../artifacts/contracts/current/core/VRFManagerWrapperV2.sol/VRFManagerWrapperV2.json');
        const artifact = JSON.parse(fs.readFileSync(artifactPath, 'utf8'));
        
        // 部署合約
        console.log("🔧 部署 VRFManagerWrapperV2...");
        ...
        console.log("✅ VRFManagerWrapperV2 已部署到:", address);
        ...
        const deploymentPath = path.join(__dirname, 'vrf-wrapper-v2-deployment.json');
        fs.writeFileSync(deploymentPath, JSON.stringify(deploymentInfo, null, 2));
        console.log("\n📝 部署信息已保存到:", deploymentPath);
        
        console.log("\n🎉 部署完成！");
        console.log("\n⚠️ 下一步：");
        console.log("1. 更新 Hero 合約的 vrfManager 地址為:", address);
        console.log("2. 更新 Relic 合約的 vrfManager 地址為:", address);
        console.log("3. 更新 AltarOfAscension 合約的 vrfManager 地址為:", address);
        console.log("4. 更新 DungeonMaster 合約的 vrfManager 地址為:", address);
        console.log("5. 測試鑄造功能");
...
main()
    .then((address) => {
        console.log("\n✅ 新的 VRFManagerWrapperV2 地址:", address);
        console.log("\n🔧 請執行以下腳本更新所有合約：");
        console.log("node scripts/active/update-vrf-manager-address.js", address);
        process.exit(0);
    })
    .catch((error) => {
        console.error(error);
