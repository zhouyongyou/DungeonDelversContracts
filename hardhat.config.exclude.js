// hardhat.config.exclude.js - 支援排除特定資料夾的配置範例
require("@nomicfoundation/hardhat-toolbox");
require("dotenv").config();

// 覆寫編譯任務
task("compile", "Compiles the entire project, excluding old folders", async (args, hre, runSuper) => {
  const fs = require("fs");
  const path = require("path");
  
  // 暫時移動 old 資料夾
  const currentDir = path.join(__dirname, "contracts/current");
  const oldDir = path.join(currentDir, "old");
  const tempDir = path.join(__dirname, "contracts/temp_old");
  ...
  try {
    ...
    // 恢復 old 資料夾
    if (movedOld && fs.existsSync(tempDir)) {
      console.log("恢復 old 資料夾...");
      fs.renameSync(tempDir, oldDir);
    }
  }
});
